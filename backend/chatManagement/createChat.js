...
const createChat = async (user1Id, user2Id) => {
	...
	try {
		...
		if (!user1Exists) {
			return { message: `User with ID ${user1Id} doesn't exist.`, status: "UserAbsence" };
		}
		if (!user2Exists) {
			return { message: `User with ID ${user2Id} doesn't exist.`, status: "UserAbsence" };
		}

	} catch (error) {
		console.error(`Error checking users :- ${error.message}`);
		return { message: `Unable to check user existence.`, status: "UserCheckError" };
	}
	...
	try {
		...
		return { message: `Unable to create new chat for ${user1Id} and ${user2Id}.`, status: "ChatCreationError" };
	}

	//add chat to user1 and user2 chats array
	const chatAddedToUser1 = await addChatToUser(chatId, user1Id);
	if (!chatAddedToUser1) {
		return { message: `Unable to add chat ${chatId} to user ${user1Id}.`, status: "ChatAdditionError" };
	}
	...
	return { chatId, message: `Added chat ${chatId} to users ${user1Id} and ${user2Id}.`, status: "ChatCreationSuccess" };
}
