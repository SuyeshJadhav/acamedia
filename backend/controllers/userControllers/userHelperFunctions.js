...
const getUserData = async (email, userId) => {
  ...
  if (email && userId)
    return {
      ...
      status: "UserSearchFail",
    };
  else if (!email && !userId)
    return { message: "Insufficient parameters", status: "UserSearchFail" };

  const userData = email
    ? await getDataByEmail(email)
    : await getDataById(userId);
  if (!userData) {
    return {
...
module.exports = { getUserData, getDataById };
